...
//création d'un sujet
  ...
  rechercherParNom(nom:string):Observable<string[]>  { 
     return this._http.get<string[]>(`${this.url_back}${this.lenom}${nom}`, {"withCredentials": true});
  }
  ...
  recupererCollegueCourant(matricule:string):Observable<Collegue> {
    return this._http.get<Collegue>(`${this.url_back}${matricule}`, {"withCredentials": true}).pipe(tap((collegue)=>this.subject.next(collegue)))
  }
  ...
  modifierEmail(matricule:string, email:string){
    return this._http.patch<Collegue>(`${this.url_back}${matricule}`,{"email":email}, {"withCredentials": true}); 
  }
 //cette méthode me permet de modifier l'email d'un collegue en fournissant dans le corps de la requête une photoUrl: dans le champ input en l'occurrence
  modifierPhotoUrl(matricule:string,photoUrl:string){
    return this._http.patch<Collegue>(`${this.url_back}${matricule}`,{"photoUrl":photoUrl}, {"withCredentials": true}); 
  }
...
//cette méthode me permet de sauvegarder un nouveau collegue en lui fournissant dans le corps de la requête un individu "monsieur"
  sauvegarderUnCollegue(monsieur:SauvegarderCollegue){
    return this._http.post<Collegue>(`${this.url_back}`,monsieur, {"withCredentials": true});
  }
//pas terminé cette verif de l'email, 
emailExists(email:string){
  return this._http.get<Collegue>(`${this.url_back}${this.emailverif}${email}`, {"withCredentials": true})
...
afficherInfo() : Observable<infoCollegue[]>{
  return this._http.get<infoCollegue[]>(`${this.url_back}${this.photos}`, {"withCredentials": true})
}
